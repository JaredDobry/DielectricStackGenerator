%LongWavePass.m
function LWPSolution = LongWavePass(lowExclB, lowExclT, inclB, inclT, upExclB, upExclT, exclConstraint, inclConstraint)
main = figure();
b = figure();
%Initial conditions
...
while done == false
    ...
    %Rank all members
    [Population, newBestFitness, avgFitness] = RankLWP(Population, lowExclB, lowExclT, inclB, inclT, exclConstraint, inclConstraint);
    if newBestFitness >= fitnessConstraint
        done = true;
        LWPSolution = Population(end,:);
    elseif newBestFitness == oldBestFitness
        same = same + 1;
    else
        same = 0;
    end
    if same >= 70 %no real progress, make change or end sim
