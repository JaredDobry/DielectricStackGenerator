...
%For Short wave pass solution finding only
function [sortedPopulation, bestFitness, avgFitness] = RankSWP(Population, inclB, inclT, upExclB, upExclT, exclConstraint, inclConstraint)
    %Get fitness for each population member
    parfor i = 1:size(Population,1)
        F(i,:) = [FitnessSWP(nonzeros(Population(i,:))', inclB, inclT, upExclB, upExclT, exclConstraint, inclConstraint, false) i];
    end
    %Sort the population
    it = 1;
