...
%For long wave pass solution finding only
function [sortedPopulation, bestFitness, avgFitness] = RankLWP(Population, lowExclB, lowExclT, inclB, inclT, exclConstraint, inclConstraint)
    %Get fitness for each population member
    parfor i = 1:size(Population,1)
        F(i,:) = [FitnessLWP(nonzeros(Population(i,:))', lowExclB, lowExclT, inclB, inclT, exclConstraint, inclConstraint, false) i];
    end
    %Sort the population
    it = 1;
